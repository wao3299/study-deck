...
- ホーム画面のヘッダーに「Google でログイン」ボタンを追加。ログイン中はアバター +
  名前 + ログアウトボタンを表示（共通コンポーネント `AuthBox.astro`。全 deck 共通）
- トップページ（deck 一覧）のヘッダーにも同じログイン UI を表示する。トップページは
  ログインのみで、統計同期は各 deck ページを開いたときに行われる（認証 UI は
  `auth-ui.js` に分離し、`sync.js` はその上に Firestore 同期を重ねる構造）
